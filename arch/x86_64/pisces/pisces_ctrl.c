	  ...
	  const char __user * ubuf, size_t size, loff_t * off)
{
	...
	if (!resp) {
		printk(KERN_ERR
		       "Unable to allocate buffer for command response\n");
		return -ENOMEM;
	}
...
cmd_read(struct file *filp, char __user * ubuf, size_t size, loff_t * off)
{
	...
	while (cmd_data == NULL) {
		wait_event_interruptible(user_waitq, (cmd_data != NULL));
	}
...
static unsigned int cmd_poll(struct file *filp, struct poll_table_struct *table)
{
	unsigned int mask = 0;
...
static int cmd_open(struct inode *inodep, struct file *filp)
{
...
static int cmd_close(struct file *filp)
{
...
static long cmd_ioctl(struct file *filp, unsigned int ioctl, unsigned long arg)
{
	...
	case PISCES_STAT_FILE:{
			...
			if (copy_from_user
			    (&file_info, uptr,
			     sizeof(struct pisces_user_file_info))) {
				printk(KERN_ERR
				       "Unable to copy to file info from user space\n");
				return -1;
			}
			...
			if (copy_from_user
			    (file_path,
			     uptr + sizeof(struct pisces_user_file_info),
			     file_info.path_len)) {
				printk(KERN_ERR
				       "Unable to copy file path from user space\n");
				kmem_free(file_path);
				return -1;
			}
	...
	case PISCES_LOAD_FILE:{
			...
			if (aspace_virt_to_phys
			    (current->aspace->id, file_info.user_addr,
			     &addr_pa) < 0) {
				printk(KERN_ERR
				       "Invalid user address used for loading file\n");
				return -1;
			}
			...
			file_size = pisces_file_size(file_handle);
			bytes_read =
			    pisces_file_read(file_handle, __va(addr_pa),
					     file_size, 0);
			...
			if (bytes_read != file_size) {
				printk(KERN_ERR
				       "Could not load file (%s) [only read %ld bytes]\n",
...
static void cmd_handler(u8 * data, u32 data_len, void *priv_data)
{
	cmd_data = data;
	cmd_len = data_len;
...
static int pisces_ctrl_init(void)
{

	waitq_init(&(user_waitq));

	kfs_create("/dev/pisces", NULL, &pisces_ctrl_fops, 0777, NULL, 0);

	xbuf_desc = pisces_xbuf_server_init((uintptr_t)
					    __va(pisces_boot_params->
						 control_buf_addr),
					    pisces_boot_params->
					    control_buf_size, cmd_handler, NULL,
					    -1, 0);
