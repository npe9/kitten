...
static void reset_flags(struct pisces_xbuf *xbuf)
{
	...
	__asm__ __volatile__("lock andq %1, %0;":"+m"(xbuf->flags)
			     :"r"(flags)
			     :"memory");
...
{
	__asm__ __volatile__("lock xchgq %1, %0;":"+m"(xbuf->flags),
			     "+r"(new_flags)
			     ::"memory");
...
{
	__asm__ __volatile__("lock orq %1, %0;":"+m"(xbuf->flags)
			     :"r"(flags)
			     :"memory");
}
...
static u32 send_data(struct pisces_xbuf *xbuf, u8 * data, u32 data_len)
{
	...
	while (bytes_left > 0) {
		u32 staged_len =
		    (bytes_left > xbuf_size) ? xbuf_size : bytes_left;
		...
		while (xbuf->staged == 1) {
			...
			if (!xbuf->ready) {
				printk(KERN_ERR
				       "XBUF disabled during data transfer\n");
				return 0;
			}
...
pisces_xbuf_sync_send(struct pisces_xbuf_desc *desc,
		      u8 * data, u32 data_len, u8 ** resp_data, u32 * resp_len)
{
	...
	while (acquired == 0) {
		...
		spin_lock_irqsave(&(desc->xbuf_lock), flags);
		{
			...
			if (!xbuf->ready) {
				printk(KERN_ERR
				       "Attempted to send to unready xbuf\n");
				spin_unlock_irqrestore(&(desc->xbuf_lock),
						       flags);
				goto err;
			}
		...
		if (!acquired) {
			wait_event_interruptible(desc->xbuf_waitq,
						 (xbuf->pending == 0));
		}
	}
...
int pisces_xbuf_complete(struct pisces_xbuf_desc *desc, u8 * data, u32 data_len)
{
	...
	if (xbuf->active == 0) {
		printk(KERN_ERR
		       "Error: Attempting to complete an inactive xbuf\n");
		return -1;
	}
	//    xbuf->active = 0;
...
static irqreturn_t ipi_handler(int irq, void *dev_id)
{
	...
	if (xbuf->active == 1) {
		printk(KERN_ERR
		       "Error IPI for active xbuf, this should be impossible\n");
		return IRQ_NONE;
	}
...
struct pisces_xbuf_desc *pisces_xbuf_server_init(uintptr_t xbuf_va,
						 u32 xbuf_total_bytes,
						 void (*recv_handler) (u8 *
								       data,
								       u32
								       data_len,
								       void
						 ...
						 u32 ipi_vector, u32 target_cpu)
{
	...
	if (ipi_vector == -1) {
		ipi_vector =
		    irq_request_free_vector(ipi_handler, 0, "XBUF_IPI", desc);

		if (ipi_vector == -1) {
			printk(KERN_ERR
			       "Could not allocate free IRQ vector for XBUF IPI\n");

			kmem_free(desc);
			return NULL;
		}
	} else {
		if (irq_request(ipi_vector, ipi_handler, 0, "XBUF_IPI", desc) ==
		    -1) {
			printk(KERN_ERR
			       "Could not register handler on IRQ %d for XBUF IPI\n",
			       ipi_vector);
			kmem_free(desc);
			return NULL;
		}
...
int pisces_xbuf_server_deinit(struct pisces_xbuf_desc *xbuf_desc)
{
...
struct pisces_xbuf_desc *pisces_xbuf_client_init(uintptr_t xbuf_va,
						 u32 ipi_vector, u32 target_cpu)
{
	struct pisces_xbuf *xbuf = (struct pisces_xbuf *)xbuf_va;
	struct pisces_xbuf_desc *desc =
	    kmem_alloc(sizeof(struct pisces_xbuf_desc));
...
int pisces_xbuf_disable(struct pisces_xbuf_desc *desc)
{
	struct pisces_xbuf *xbuf = desc->xbuf;
...
int pisces_xbuf_enable(struct pisces_xbuf_desc *desc)
{
	struct pisces_xbuf *xbuf = desc->xbuf;
