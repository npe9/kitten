...
u64 pisces_file_open(const char *path, int mode)
{
	...
	pisces_lcall_exec((struct pisces_lcall *)open_lcall,
			  (struct pisces_lcall_resp **)&open_resp);
...
int pisces_file_close(u64 file_handle)
{
	...
	close_lcall.lcall.data_len =
	    sizeof(struct vfs_close_lcall) - sizeof(struct pisces_lcall);
	close_lcall.file_handle = file_handle;

	pisces_lcall_exec((struct pisces_lcall *)&close_lcall,
			  (struct pisces_lcall_resp **)&close_resp);
...
loff_t pisces_file_size(u64 file_handle)
{
	...
	size_lcall.lcall.data_len =
	    sizeof(struct vfs_size_lcall) - sizeof(struct pisces_lcall);
	size_lcall.file_handle = file_handle;

	pisces_lcall_exec((struct pisces_lcall *)&size_lcall,
			  (struct pisces_lcall_resp **)&size_resp);
...
ssize_t
pisces_file_read(u64 file_handle, void *buffer, size_t length, loff_t offset)
{
	struct vfs_read_lcall *read_lcall = NULL;
	struct pisces_lcall_resp *read_resp = NULL;

	u32 lcall_len =
	    sizeof(struct vfs_read_lcall) + sizeof(struct vfs_buf_desc);
	u64 ret = 0;
	...
	pisces_lcall_exec((struct pisces_lcall *)read_lcall,
			  (struct pisces_lcall_resp **)&read_resp);
...
ssize_t
pisces_file_write(u64 file_handle, void *buffer, size_t length, loff_t offset)
{
	struct vfs_write_lcall *write_lcall = NULL;
	struct pisces_lcall_resp *write_resp = NULL;

	u32 lcall_len =
	    sizeof(struct vfs_write_lcall) + sizeof(struct vfs_buf_desc);
	u64 ret = 0;
	...
	write_lcall->lcall.data_len =
	    sizeof(struct vfs_write_lcall) - sizeof(struct pisces_lcall);
	...
	pisces_lcall_exec((struct pisces_lcall *)write_lcall,
			  (struct pisces_lcall_resp **)&write_resp);
