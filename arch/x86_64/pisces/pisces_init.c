...
void __init pisces_init(char *real_mode_data)
{

	pisces_boot_params =
	    (struct pisces_boot_params *)__va((u64) real_mode_data <<
					      PAGE_SHIFT);
	...
	memcpy(lwk_command_line, pisces_boot_params->cmd_line,
	       sizeof(lwk_command_line));
