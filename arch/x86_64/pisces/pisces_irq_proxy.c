...
static void send_ipi_from_msi(struct msi_msg *msg)
{
	...
	unsigned int icr = ((msg->vector << 0) |
			    (msg->delivery_mode << 8) |
			    (msg->level << 14) | (msg->trigger_mode << 15));

	lapic_issue_raw_ipi(msg->dest, icr);
}

static irqreturn_t pisces_interrupt_handler(int vector, void *priv)
{
...
static int setup_msi_proxy(struct pisces_irq_proxy *proxy)
{
	...
	if ((hdr->msi.valid == 0) ||
	    ...
	    ((hdr->msi.msi_ctrl & PCIM_MSICTRL_MSI_ENABLE) == 0)) {
		printk(KERN_ERR
		       "Error: only support single vector MSI device\n");
		return -1;
	}
...

	{
		...
		debug
		    ("  address_lo %x (dest %u, redirect_hint %u, dest_mode %u)\n",
		     msg->address_lo, msg->dest, msg->redirect_hint,
		     msg->dest_mode);
		debug
		    ("  data %x (vector %u, delivery_mode %u, level %u, trigger_mode %u)\n",
		     msg->data, msg->vector, msg->delivery_mode, msg->level,
		     msg->trigger_mode);

		if ((msg->redirect_hint != 0) || (msg->dest_mode != 0)) {
			printk
			    ("Error: unsupported MSI mode (redirect_hint %u, dest_mode %u)",
	...
	compose_msi_msg(&(proxy->new_msg), cpu_info[0].arch.apic_id,
			proxy->vector);
	debug("new_msi_msg: dest %d, vector %d\n", cpu_info[0].arch.apic_id,
	      proxy->vector);

	write_msi_msg(proxy->dev, &(proxy->new_msg));

	// request irq
	irq_request(proxy->vector, &pisces_interrupt_handler, 0, "irq-proxy",
		    proxy->priv);
...
int
pisces_setup_irq_proxy(pci_dev_t * pci_dev,
		       irq_handler_t handler, int vector, void *priv)
{
