...
static int
push_active_entry(struct xpmem_cmd_ringbuf *buf, struct xpmem_cmd_ex *cmd)
{
...
static void
pop_active_entry(struct xpmem_cmd_ringbuf *buf, struct xpmem_cmd_ex *cmd)
{
	unsigned long flags = 0;
	int start, end;
...
static void xpmem_ctrl_handler(u8 * data, u32 data_len, void *priv_data)
{
	struct pisces_xpmem_state *state =
	    (struct pisces_xpmem_state *)priv_data;
	...
	if (push_active_entry(buf, (struct xpmem_cmd_ex *)data) != 0) {
		printk(KERN_ERR
		       "ERROR: DROPPING XPMEM CMD: NO ENTRIES AVAILABLE\n");
		return;
	}
...
static int
pisces_xpmem_lcall_send(struct pisces_xpmem_cmd_lcall *xpmem_lcall, u64 size)
{
...
static int xpmem_cmd_fn(struct xpmem_cmd_ex *cmd, void *priv_data)
{
	...
	/* Perform xbuf send */
	return pisces_xpmem_lcall_send(&lcall,
				       sizeof(struct pisces_xpmem_cmd_lcall));
}

static int
xpmem_segid_fn(xpmem_segid_t segid,
	       xpmem_sigid_t sigid, xpmem_domid_t domid, void *priv_data)
{
...
static void xpmem_kill_fn(void *priv_data)
{
	struct pisces_xpmem_state *state =
	    (struct pisces_xpmem_state *)priv_data;
...
static int map_domain_region(u64 pfn_pa, u64 size)
{
	paddr_t paddr_start;
	paddr_t paddr_end;
...
static int xpmem_thread_fn(void *private)
{
	...
	while (1) {
		/* Wait on waitq */
		wait_event_interruptible(state->waitq,
					 ((atomic_read(&(buf->active_entries)) >
					   0) || (should_exit == 1)
					 )
		    );
		...
		if (cmd.type == XPMEM_ATTACH) {
			if (map_domain_region
			    (cmd.attach.pfn_pa,
			     (cmd.attach.num_pfns * sizeof(u32))) != 0) {
				printk(KERN_ERR
				       "Cannot map domain pfn list into kernel\n");
				continue;
			}
		}
...
xpmem_link_t pisces_xpmem_init(void)
{
	...
	state->link = xpmem_add_connection(state,
					   xpmem_cmd_fn,
					   xpmem_segid_fn, xpmem_kill_fn);
	...
	state->xbuf_desc = pisces_xbuf_server_init((uintptr_t)
						   __va(pisces_boot_params->
							xpmem_buf_addr),
						   pisces_boot_params->
						   xpmem_buf_size,
						   xpmem_ctrl_handler, state,
						   -1, 0);
	...
	state->xpmem_thread = kthread_run(xpmem_thread_fn,
					  state, "xpmem-thread");
...
void pisces_xpmem_deinit(xpmem_link_t link)
{
