...
static int send_resp(int fd, u64 err_code)
{
...
static int issue_vm_cmd(int vm_id, u64 cmd, uintptr_t arg)
{
	int palacios_fd = 0;
	char vm_fname[128];
...
static int issue_v3_cmd(u64 cmd, uintptr_t arg)
{
	int palacios_fd = 0;
	int ret = 0;
...
static int load_file(int pisces_fd, char *lnx_file, char *lwk_file)
{
	struct pisces_user_file_info *file_info = NULL;
	int path_len = strlen(lnx_file) + 1;
	size_t file_size = 0;
	char *file_buf = NULL;
	printf("mallocing file\n");
	...
	printf("freed the buf\n");
	free(file_buf);

	return 0;
}

static int launch_job(int pisces_fd, struct pisces_job_spec *job_spec)
{
	...
	/* Load exe file info memory */
	{
		...
		file_info =
		    malloc(sizeof(struct pisces_user_file_info) + path_len);
		memset(file_info, 0,
		       sizeof(struct pisces_user_file_info) + path_len);
...

		{
			...
			paddr_t pmem =
			    elf_dflt_alloc_pmem(file_size, page_size, 0);

			printf
			    ("PMEM Allocated at %p (file_size=%lu) (page_size=0x%x) (pmem_size=%p)\n",
			     (void *)pmem, file_size, page_size,
			     (void *)round_up(file_size, page_size));

			if (pmem == 0) {
				printf
				    ("Could not allocate space for exe file\n");
				return -1;
			}

			/* Map the segment into this address space */
			status =
			    aspace_map_region_anywhere(my_aspace_id,
						       &file_addr,
						       round_up(file_size,
								page_size),
						       (VM_USER | VM_READ |
	...
	printf("Job Launch Request (%s) [%s %s]\n", job_spec->name,
	       job_spec->exe_path, job_spec->argv);
	...
	/* Initialize start state for each rank */
	{
		...
		start_state =
		    malloc(job_spec->num_ranks * sizeof(start_state_t));
		if (!start_state) {
		...
		for (rank = 0; rank < job_spec->num_ranks; rank++) {
			...
			status = elf_load((void *)file_addr, job_spec->name, ANY_ID, page_size, job_spec->heap_size,	// heap_size 
					  job_spec->stack_size,	// stack_size 
					  job_spec->argv,	// argv_str
					  job_spec->envp,	// envp_str
					  &start_state[rank],
					  0, &elf_dflt_alloc_pmem);

			if (status) {
				printf("elf_load failed, status=%d\n", status);
			}

			if (aspace_update_user_cpumask
			    (start_state[rank].aspace_id, &lwk_cpumask) != 0) {
			...
			if (job_spec->use_smartmap) {
				...
				for (dst = 0; dst < job_spec->num_ranks; dst++) {
					for (src = 0; src < job_spec->num_ranks;
					     src++) {
						status =
						    aspace_smartmap(start_state
								    [src].
								    aspace_id,
								    start_state
								    [dst].
...
int main(int argc, char **argv, char *envp[])
{
	...
	while (1) {
		...
		case ENCLAVE_CMD_ADD_MEM:{
				...
				ret =
				    read(pisces_fd, &mem_cmd,
					 sizeof(struct cmd_mem_add));

				if (ret != sizeof(struct cmd_mem_add)) {
					printf
					    ("Error reading pisces MEM_ADD CMD (ret=%d)\n",
					     ret);
		...
		case ENCLAVE_CMD_ADD_CPU:{
				...
				ret =
				    read(pisces_fd, &cpu_cmd,
					 sizeof(struct cmd_cpu_add));

				if (ret != sizeof(struct cmd_cpu_add)) {
					printf
					    ("Error reading pisces CPU_ADD CMD (ret=%d)\n",
					     ret);
				...
				if (issue_v3_cmd
				    (V3_ADD_CPU,
				     (uintptr_t) logical_cpu) == -1) {
					printf
					    ("Error: Could not add CPU to Palacios\n");
				}
		...
		case ENCLAVE_CMD_REMOVE_CPU:{
				...
				printf
				    ("Removing CPU phys_id %llu, apic_id %llu\n",
				     (unsigned long long)cpu_cmd.phys_cpu_id,
				     (unsigned long long)cpu_cmd.apic_id);

				logical_cpu =
				    phys_cpu_remove(cpu_cmd.phys_cpu_id,
						    cpu_cmd.apic_id);
		...
		case ENCLAVE_CMD_LAUNCH_JOB:{
				struct cmd_launch_job *job_cmd =
				    malloc(sizeof(struct cmd_launch_job));
				int ret = 0;

				memset(job_cmd, 0,
				       sizeof(struct cmd_launch_job));

				ret =
				    read(pisces_fd, job_cmd,
					 sizeof(struct cmd_launch_job));

				if (ret != sizeof(struct cmd_launch_job)) {
					printf
					    ("Error reading Job Launch CMD (ret = %d)\n",
					     ret);
		...
		case ENCLAVE_CMD_LOAD_FILE:{
				struct cmd_load_file *load_cmd =
				    malloc(sizeof(struct cmd_load_file));
				int ret = 0;

				memset(load_cmd, 0,
				       sizeof(struct cmd_load_file));

				ret =
				    read(pisces_fd, load_cmd,
					 sizeof(struct cmd_load_file));

				if (ret != sizeof(struct cmd_load_file)) {
					printf
					    ("Error reading LOAD FILE CMD (ret = %d)\n",
					     ret);
		...
		case ENCLAVE_CMD_CREATE_VM:{
				...
				memset(&vm_cmd, 0,
				       sizeof(struct cmd_create_vm));
				memset(&rgn, 0, sizeof(struct pmem_region));
				memset(&guest_img, 0,
				       sizeof(struct v3_guest_img));

				ret =
				    read(pisces_fd, &vm_cmd,
					 sizeof(struct cmd_create_vm));

				if (ret != sizeof(struct cmd_create_vm)) {
					send_resp(pisces_fd, -1);
					printf
					    ("Error: CREATE_VM command could not be read\n");
					break;
				}

				path_len =
				    strlen((char *)vm_cmd.path.file_name) + 1;

				file_info =
				    malloc(sizeof(struct pisces_user_file_info)
					   + path_len);
				memset(file_info, 0,
				       sizeof(struct pisces_user_file_info) +
				       path_len);

				file_info->path_len = path_len;
				strncpy(file_info->path,
					(char *)vm_cmd.path.file_name,
					path_len - 1);

				file_size =
				    ioctl(pisces_fd, PISCES_STAT_FILE,
					  file_info);
				...
				guest_img.guest_data =
				    (void *)file_info->user_addr;
				strncpy(guest_img.name,
					(char *)vm_cmd.path.vm_name, 127);

				/* Issue VM Create command to Palacios */
				vm_id =
				    issue_v3_cmd(V3_CREATE_GUEST,
						 (uintptr_t) & guest_img);

				aspace_unmap_region(my_aspace_id, file_addr,
						    round_up(file_size,
							     PAGE_SIZE));
				pmem_free_umem(&rgn);
				...
				if (vm_id < 0) {
					printf
					    ("Error: Could not create VM (%s) at (%s) (err=%d)\n",
					     vm_cmd.path.vm_name,
					     vm_cmd.path.file_name, vm_id);
		...
		case ENCLAVE_CMD_ADD_V3_PCI:{
				...
				if (issue_v3_cmd
				    (V3_ADD_PCI,
				     (uintptr_t) & (v3_pci_spec)) == -1) {
					printf
					    ("Error: Could not add PCI device to Palacios\n");
					send_resp(pisces_fd, -1);
					break;
				}
		...
		case ENCLAVE_CMD_FREE_V3_PCI:{
				...
				if (issue_v3_cmd
				    (V3_REMOVE_PCI,
				     (uintptr_t) & (v3_pci_spec)) == -1) {
					printf
					    ("Error: Could not remove PCI device from Palacios\n");
					send_resp(pisces_fd, -1);
					break;
				}
		...
		case ENCLAVE_CMD_VM_CONS_CONNECT:{
				...
				if (issue_vm_cmd
				    (vm_cmd.vm_id, V3_VM_CONSOLE_CONNECT,
		...
		case ENCLAVE_CMD_VM_CONS_DISCONNECT:{
				...
				if (issue_vm_cmd
				    (vm_cmd.vm_id, V3_VM_CONSOLE_DISCONNECT,
				     (uintptr_t) NULL) == -1) {
					send_resp(pisces_fd, -1);
					break;
				}
		...
		case ENCLAVE_CMD_VM_CONS_KEYCODE:{
				...
				ret =
				    read(pisces_fd, &vm_cmd,
					 sizeof(struct cmd_vm_cons_keycode));
				...
				if (issue_vm_cmd
				    (vm_cmd.vm_id, V3_VM_KEYBOARD_EVENT,
				     vm_cmd.scan_code) == -1) {
					send_resp(pisces_fd, -1);
					break;
				}
		...
		case ENCLAVE_CMD_VM_DBG:{
				...
				ret =
				    read(pisces_fd, &pisces_cmd,
					 sizeof(struct cmd_vm_debug));
				...
				if (issue_vm_cmd
				    (pisces_cmd.spec.vm_id, V3_VM_DEBUG,
				     (uintptr_t) & v3_cmd) == -1) {
					send_resp(pisces_fd, -1);
					break;
				}
		...
		case ENCLAVE_CMD_SHUTDOWN:{
				...
				if (issue_v3_cmd(V3_SHUTDOWN, 0) == -1) {
					printf
					    ("Error: Could not shutdown Palacios VMM\n");
					send_resp(pisces_fd, -1);
					break;
				}
